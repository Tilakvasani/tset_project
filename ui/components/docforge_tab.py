...
def render_docforge_tab(api_get, api_post, DOCX_AVAILABLE, build_docx, init_session, sidebar_progress_bar=None):
    """
    Renders the DocForge AI (Generator) interface.
    Extracted from the main streamlit_app.py monolith.
    Includes Step 1 (Setup), Step 2 (Questions), Step 3 (Answers), 
    Step 4 (Review/Edit), and Step 5 (Export).
    """
    ...
    # ── Step 4: Review & Edit ──────────────────────────────────────────────────
        ...
        with right:
            ...
            st.write("")
            instr = st.text_area("AI Edit Instruction", placeholder="e.g. Make more formal", height=60, key="edit_instr", label_visibility="collapsed")
    ...
    # ── Step 5: Export ─────────────────────────────────────────────────────────
        ...
        with col_d:
            with st.container(border=True):
                ...
                if DOCX_AVAILABLE:
                    if st.session_state.get("docx_bytes_cache") is None or st.session_state.get("docx_cache_doc") != doc_type:
                        try:
                    ...
                    if st.session_state.get("docx_bytes_cache"):
                        st.download_button("⬇️ Download .docx", data=st.session_state.docx_bytes_cache, file_name=f"{safe}.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", use_container_width=True, type="primary")
                
                st.download_button("⬇️ Download .txt", data=full_doc, file_name=f"{safe}.txt", mime="text/plain", use_container_width=True)
